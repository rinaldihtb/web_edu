<?php 
	class ModelNewsBerita
	{
		...
		public function getList($recently_post=false) {
			...
			if($recently_post) {
				$result = $konek->query("SELECT * FROM news_post ORDER BY id LIMIT 5");
			} else {
				$result = $konek->query("SELECT * FROM news_post");
			}
			$results = array();
			while($row = $result->fetch_assoc()) {
				...
				$row['href'] = "http://".$_SERVER['HTTP_HOST']."/index.php?route=news/news/newsItem&id=$row[id]";
		...
		public function getBerita($id=0) {
			$con = new conn;
			$konek = $con->open();
			$result = $konek->query("SELECT * FROM news_post  WHERE id='$id'");
